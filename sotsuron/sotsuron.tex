...
\chapter{結果}
  ...
  \section{リング半径・チェレンコフ角}
    ...
    \subsection{リング半径・チェレンコフ角の絶対値のズレ}
      ...
      図()のように、MPPCが焦点面からズレていた場合焦点面で収束されるはずの光が内側と外側に広がってしまう。上流側の光が多いほど内側にくる光が多くなる。今回のセットアップによるズレを考慮して計算を行うと表$\ref{tab:radius}$のようになった。
    ...
    これは、暗電流と重複hitの効果が原因と考えられる。
    先ほども書いたように、暗電流にはリング半径を大きくする効果があり、電圧を上げていくと暗電流が増加していくので、リング半径・チェレンコフ角が増加していく。
  ...
  \section{角度分解能}
    ...
    1 p.e.あたりの分解能を求めるため、イベントごとのhit数で場合わけをし分解能を求めた。
    図\ref{fig:per_hit}のグラフを1 p.e.あたりの分解能を$\Delta \theta_{1 p.e.}$として、$\frac{\Delta \theta_{1 p.e.}}{\sqrt{nhits}}$でフィットした。
    フィッティングの値より、1 p.e.あたりの分解能$\Delta \theta_{1 p.e.} = 4.85 \pm 0.01 \: \si{mrad}$と求められた。
    \begin{figure}[h]
      \begin{center} 
    ...
    暗電流の角度分解能への影響を調べるために、TDCのcut幅を100 chまで20 chずつ広げていき暗電流の割合を増加させた時の分解能を調べた。
